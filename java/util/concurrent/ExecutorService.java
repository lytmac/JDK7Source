...
public interface ExecutorService extends Executor {
     ...
     *
     * If you would like to immediately block waiting for a task, you can use constructions of the form result = exec.submit(Callable.get());
     *
    ...
    /**
     * Runnable 对象封装的任务并不会有返回值，同时我们又期望利用到Future的阻塞等待执行成功的效果，可以调用这个方法，该方法会将传入的result放入Future，并在执行成功后返回。
    ...
    /**
     * 任务成功完成后返回对象future，调用 future.get() 只会返回null。在我们期望主线程阻塞等待Runnable子任务完成后才继续执行的情况下可以使用该方法
     ...
     * Executes the given tasks, returning the result of one that has completed successfully (without throwing an exception), if any do.
     *
     * 返回时还未执行的任务将被取消
     ...
     * @throws InterruptedException if interrupted while waiting
     * @throws NullPointerException if tasks or any element task subject to execution is null
     ...
     * @return the result returned by one of the tasks.
     * @throws InterruptedException if interrupted while waiting
     * @throws NullPointerException if tasks, or unit, or any element task subject to execution is <tt>null</tt>
